<article id="comment{!! $comment->id !!}" class="comment">
    <header>
        <h2>{!! trans('comments::written_by') !!} {!! link_to('users/'.$comment->creator->id, $comment->creator->username, ['class' => 'creator-name']) !!} {!! trans('comments::written_on') !!} {!! $comment->created_at !!}</h2>
    </header>
    <div class="text">
        {!! $comment->renderText() !!}
    </div>
